...
function verInfoCuenta(codigoCuenta) {
    fetch(`CuentaServlet?action=detalles&id=${codigoCuenta}`)
        .then(response => {
            if (!response.ok) {
...
function cargarSaldo() {
    fetch(`CuentaServlet?action=detalles&id=${codigoCuentaGlobal}`)
        .then(response => {
            if (!response.ok) {
                throw new Error("Cuenta no encontrada");
            }
        ...
        .then(data => {
            document.getElementById("saldoCuenta").innerText = `$${data.saldo.toFixed(2)}`;
        })
        .catch(error => {
            console.error("Error:", error);
            document.getElementById("saldoCuenta").innerText = "Error al cargar saldo";
        });
}
